...
class RoboClawConnectionManager:
    ...
    def test_roboclaw_connection(self, port):
        ...
        try:
            ...
            # Try different addresses in case the default is wrong
            addresses_to_try = [0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87]
...
class HumanFollowingRobot:
    ...
    def follow_person(self, person: PersonInfo) -> None:
        ...
        if distance < self.PERSON_FOLLOW_DISTANCE:
            ...
            if roboclaw:
                try:
                    roboclaw.SpeedM1(self.address, -self.FORWARD_TARGET_QPPS // 2)
                    roboclaw.SpeedM2(self.address, self.FORWARD_TARGET_QPPS // 2)
            ...
            if roboclaw:
                try:
                    if lateral_error > 0:
                        ...
                        roboclaw.SpeedM1(self.address, self.TURN_QPPS // 2)
                        roboclaw.SpeedM2(self.address, -self.TURN_QPPS // 2)
                        time.sleep(0.2)
                        self.stop_both()
                    else:
                    ...
                    print(f"Error during lateral adjustment: {e}")
                    self.connection_manager.connected = False
